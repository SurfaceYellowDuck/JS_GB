...
const snake = {
    ...
    makeStep() {
        const thisPoint = this.getBody()[0];
    ...
    renderCount() {
        let counterString = document.getElementsByClassName('counterString')[0];
        counterString.innerHTML = ' ';
        counterString.textContent = `Ваш счёт равен ${this.count}`;
    },
    getNextStepHeadPoint() {
        let firstPoint = this.getBody()[this.getBody().length - 1];
        switch (this.direction) {
            case 'up':
                if (firstPoint.y == 0) {
                    return { x: firstPoint.x, y: firstPoint.y = settings.colsCount }
                }
            ...
            case 'left':
                if (firstPoint.x == 0) {
                    return { x: firstPoint.x = settings.rowsCount, y: firstPoint.y };
                }
                return { x: firstPoint.x - 1, y: firstPoint.y };
        }
    },
...
const game = {
    ...
    tickHandler() {
        if (this.obstruction.isOnPoint(this.snake.getNextStepHeadPoint())) this.finish();
